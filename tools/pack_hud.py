...
def emit(wt):
    ...
    eng = tab[:HUD_TAB_ENGINE * 7]
    hi = {eng[k * 7 + 2] for k in range(HUD_TAB_ENGINE)}
    assert len(hi) == 1, 'HUD_TAB spans VBXE banks %s -- hud_entry assumes ' 'one, and the row would have to go back to 7 B' % sorted(hi)
    open(os.path.join(out, 'hud.tab'), 'wb').write(
        bytes(b for k, b in enumerate(eng) if k % 7 != 2))
    # hud_syms.inc -- the VRAM address of the lumps that are in the BLOB but not
    # in the TABLE. HUD_TAB is capped at HUD_TAB_ENGINE entries because
    # HUDTAB_BASE..END is 240 B and the vissprite arrays sit right above it, so
    # anything past index 28 has to be addressed directly. The FPS readout
    # builds its own 7-byte record from these (hud.asm fps_dig).
    ...
    with open(syms, 'w') as f:
        ...
        for nm in ('STYSNUM0', 'STCFN044'):
            i = HUD_LUMPS.index(nm)
            o = i * 7
            a = tab[o] | tab[o + 1] << 8 | tab[o + 2] << 16
            eq = 'HUDV_YS0' if nm == 'STYSNUM0' else 'HUDV_COMMA'
            f.write('%-12s equ $%06X   ; %s %dx%d\n'
                    % (eq, a, nm, tab[o + 3], tab[o + 4]))
        i = HUD_LUMPS.index('STYSNUM0')
        ...
        j = HUD_LUMPS.index('STCFN044')
        f.write('HUDV_COMMAH  equ %d\n' % tab[j * 7 + 4])
        f.write('HUDV_COMMAT  equ %d          ; the comma sits BELOW the digit top\n'
                % (tab[j * 7 + 6] - 256 if tab[j * 7 + 6] > 127 else tab[j * 7 + 6]))
    print(f'hud.bin {len(blob)} B (VRAM ${HUD_VRAM_BASE:06X}..${addr:06X}), '
          f'hud.tab {HUD_TAB_ENGINE * 6} B ({HUD_TAB_ENGINE} of {len(tab)//7} '
          f'lumps -- the rest are blob-only, see hud_syms.inc) -> {out}')
