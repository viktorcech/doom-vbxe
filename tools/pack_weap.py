...
WEAPONS = [
    ...
    # 2026-08-28: the BFG, the last weapon with no art. S_BFG1..S_BFG4 name
    # BFGG A and B, S_BFGFLASH1/2 name BFGF A and B (info.c). It cost nothing
    # but SRAM in the end: the 2026-08-03 diet moved the master to Rapidus and
    # left VRAM ONE slot sized for the shotgun, so "its 11 KB do not fit the
    # five holes" (memory_map.inc, written against the old scattered REGIONS)
    # stopped being true the day that slot appeared.
    ('bfg',      'BFGG', 'AB',   ('BFGF', 'AB')),
]

# WEAPONS-list key -> the WP_* id (memory_map.inc / d_items.c wp_* enum). The
# copy table wp_wload indexes is WP_*-ordered.
WP_ID = {'fist': 0, 'pistol': 1, 'shotgun': 2, 'chaingun': 3,
         'missile': 4, 'plasma': 5, 'bfg': 6, 'chainsaw': 7}
WP_NWEAP = 8
...
def emit(wt):
    ...
    #      and how many 256-byte pages wp_wload moves --
    by_key = {key: (src, wlen) for key, src, wlen in weap_runs}
    wpl = [None] * WP_NWEAP
    for key, (src, wlen) in by_key.items():
        wpl[WP_ID[key]] = (key, src, (wlen + 255) // 256)
    assert None not in wpl, f'no art for WP_* {[i for i, r in enumerate(wpl) if r is None]}'
